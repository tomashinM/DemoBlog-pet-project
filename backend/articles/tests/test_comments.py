...
from utils.factories import ArticleFactory, CommentFactory
...
class TestCommentViewSet(TestMixin, APITestCase):
    ...
    def setUp(self):
        super().setUp()
        self.article = ArticleFactory.create(author=self.celeb_user)
        self.article_url = reverse_lazy(
            "article-comments-list", kwargs={"slug": self.article.slug}
        )
        ...
        self.comment_url = reverse_lazy(
            ...
            kwargs={"slug": self.article.slug, "pk": self.comment.pk},
        )
